...
app.get('/', (req: Request, res: Response) => {
    ...
    var html = fs.readFileSync("src/index.html").toString().split("${href}").join(href);
...
app.get('/products/:_id', async (req: Request, res: Response) => {
    ...
    try {
        product = await db.collection("products").find({_id: new mongoDB.ObjectId(req.params._id)}).toArray();
        product = product[0];
    } catch {
        res.json({ error: 'Not found' });
    }

    console.log(`🔎 [server]: User asked for product with _ObjectId(${req.params._id})`)
...
app.listen(port, async () => {
    ...
    if (await db.collection("products").countDocuments({}) == 0) {
        ...
        await db.collection("products").insertMany([
            {"name":"Necronomicon","description":"Big amount of unwonted content for anyone sane in the smallest possible package.","price":99.99,"rating":4.55,"stock":0},
